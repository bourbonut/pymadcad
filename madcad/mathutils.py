...
def transform(*args) -> mat4:
    ...
    if len(args) == 1:
        if isinstance(args[0], mat4):
            return args[0]
        elif isinstance(args[0], (mat3, quat, int, float)):
            return mat4(args[0])
        elif isinstance(args[0], vec3):
            return translate(args[0])
    elif len(args) == 2:
        if isinstance(args[0], vec3):
            if isinstance(args[1], (mat3, quat, int, float)):
                m = mat4(args[1])
            elif isinstance(args[1], vec3):
                m = mat4(quat(args[1]))
            m[3] = vec4(args[0], 1)
            return m
    ...
    raise TypeError(
        "a transformation must be a  mat3, mat4, quat, (O,mat3), (O,quat), (0,x,y,z), not {}".format(
            args
        )
    )
...
def transformer(trans):
    ...
    """
    if isinstance(trans, (dquat, fquat)):
        trans = mat3_cast(trans)
    if callable(trans):
        return trans
    if isinstance(trans, (dvec3, fvec3)):
        return lambda v: v + trans
    if isinstance(trans, (dmat3, fmat3, dmat4, fmat4, int, float)):
        return lambda v: trans * v
    raise TypeError(
        "a transformer must be a  vec3, quat, mat3, mat4 or callable, not {}".format(
            trans
        )
    )
...
def interpol2(a, b, x):
    ...
    """
    return (
        2 * x * (1 - x) * interpol1(a[0], b[0], x)  # linear component
...
def intri_sphere(pts, ptangents, a, b, etangents=None):
    ...
    P = (
        ...
        + b * B
        + c * C
        + a * b * c * (ta[0] + ta[1] + tb[0] + tb[1] + tc[0] + tc[1])
    )
    return (
        0
        + a**2 * (A + b * ta[0] + c * ta[1])
...
def distance_pe(pt, edge):
    ...
    if x < 0:
        return distance(pt, edge[0])
    elif x > 1:
        return distance(pt, edge[1])
    else:
...
def distance_pt(p, triangle):
    ...
    for i in range(3):
        if (
            dot(p - triangle[i - 1], cross(triangle[i - 2] - triangle[i - 1], normal))
            > 0
        ):
...
def linrange(start, stop=None, step=None, div=0, end=True):
    ...
    if stop is None:
        start, stop = 0, start
    if step is None:
        step = (stop - start) / (div + 1)
    elif step * (stop - start) < 0:
        step = -step
    if not end:
        stop -= step
    stop += NUMPREC * stop
...
class Axis(object):
    ...
    def __getitem__(self, i):
        ...
        if i == 0:
            ...
            raise IndexError("an axis has only 2 components")
    ...
    def offset(self, increment) -> "Axis":
        ...
        return Axis(
            self.origin + self.direction * increment, self.direction, self.interval
        )

    def transform(self, transform) -> "Axis":
        """move the axis by the given transformation"""
        if isinstance(transform, (float, int)):
            return Axis(transform * self.origin, self.direction, self.interval)
        elif isinstance(transform, vec3):
            return Axis(transform + self.origin, self.direction, self.interval)
        elif isinstance(transform, (mat3, quat)):
            return Axis(
                transform * self.origin,
                normalize(transform * self.direction),
                self.interval,
            )
        elif isinstance(transform, (mat4)):
            return Axis(
                transform * self.origin,
                normalize(mat3(transform) * self.direction),
                self.interval,
            )
...
def isaxis(obj):
    ...
    return (
        ...
        and len(obj) == 2
        and isinstance(obj[0], vec3)
        and isinstance(obj[1], vec3)
    )
...
class Box:
    ...
    def __init__(self, min=None, max=None, center=vec3(0), width=vec3(-inf)):
        if min and max:
            ...
            self.min, self.max = center - width / 2, center + width / 2
    ...
    def corners(self) -> "[vec3]":
        ...
        t = type(self.min)
        return [
            t(c[i & 1][0], c[(i >> 1) & 1][1], c[(i >> 2) & 1][2]) for i in range(8)
        ]
    ...
    def __add__(self, other):
        if isinstance(other, vec3):
            ...
            return Box(other.min, other.max)
        else:
    ...
    def __or__(self, other):
        return deepcopy(self).union(other)

    def __and__(self, other):
        return deepcopy(self).intersection(other)
    ...
    def union(self, other) -> "Box":
        ...
        if isinstance(other, (dvec3, fvec3)):
            ...
            return Box(glm.min(self.min, other.min), glm.max(self.max, other.max))
        else:
            raise TypeError("unable to integrate {}".format(type(other)))
    ...
    def intersection(self, other) -> "Box":
        ...
        """
        if isinstance(other, Box):
            return Box(glm.max(self.min, other.min), glm.min(self.max, other.max))
        else:
            raise TypeError("expected a Box".format(type(other)))
    ...
    def __repr__(self):
        return "{}({}, {})".format(
            self.__class__.__name__, repr(self.min), repr(self.max)
        )
...
class Screw(object):
    ...
    def __init__(self, resulting=None, momentum=None, position=None):
        self.resulting, self.momentum, self.position = (
            resulting or vec3(0),
            momentum or vec3(0),
            position or vec3(0),
        )
    ...
    def locate(self, pt) -> "Screw":
        ...
        return Screw(
            self.resulting,
            self.momentum + cross(self.resulting, pt - self.position),
            pt,
        )
    ...
    def transform(self, mat) -> "Screw":
        ...
        return Screw(
            rot * self.resulting, rot * self.momentum, rot * self.position + trans
        )
    ...
    def __add__(self, other):
        ...
        return Screw(
            self.resulting + other.resulting,
