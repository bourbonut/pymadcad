...
class Distance(Constraint):
    ...
    def fit(self):
        if isinstance(self.p1, vec3) and isinstance(self.p2, vec3):
            ...
            return (
                (
                    length(noproject(self.p2.origin - self.p1, self.p2.direction))
                    - self.d
                )
                ** 2,
            )
            ...
            return (
                (
                    length(noproject(self.p1.origin - self.p2, self.p1.direction))
                    - self.d
                )
                ** 2,
            )
            ...
            return length2(cross(d1, d2)), (
                length(noproject(self.p1.origin - self.p2.origin, d1 + d2)) - self.d
            ) ** 2
    ...
    def display(self, scene):
        ...
        if isinstance(self.p1, vec3) and isinstance(self.p2, vec3):
            ...
            return scene.display(
                scheme.note_distance(
                    self.p1,
                    self.p1 + noproject(self.p2.origin - self.p1, self.p2.direction),
                    text="{:.5g}\n{:+.1g}".format(self.d, sqrt(self.fit()[0])),
                )
            )
        elif isinstance(self.p2, vec3):
            return scene.display(
                scheme.note_distance(
                    self.p2,
                    self.p2 + noproject(self.p1.origin - self.p2, self.p1.direction),
                    text="{:.5g}\n{:+.1g}".format(self.d, sqrt(self.fit()[0])),
                )
            )
        else:
    ...
    def display(self, scene):
        return scene.display(
            scheme.note_angle(
                ...
                text="{:.5g}°\n{:+.1g}".format(
                    degrees(self.angle), degrees(sqrt(self.fit()[0]))
                ),
            )
        )
...
class Problem:
            ...
            if callable(obj.slvvars):
                for var in obj.slvvars():
                    if isinstance(var, (float, int)):
                        raise TypeError(
                            "primitive types (float,int) are not allowed when 'slvvars' is a callable"
                        )
                    self.register(var)
            else:
    ...
    def solve(self, precision=1e-6, method="trf", maxiter=None, afterset=None):
        # nprint(self.slvvars)
        if afterset:
            evaluate = lambda x: afterset(x) or self.evaluate(x)
        else:
    ...
    while maxdelta > precision:
        ...
        for const, part in zip(constraints, parts):
            ...
            for correction in corr:
                ...
                l = length(contrib)
                if l > corrnorms[indices[i]]:
                    corrnorms[indices[i]] = l
                i += 1

        # apply changes
        for param, correction, corrnorm, oldcorr in zip(
            params, corrections, corrnorms, oldcorrs
        ):
            if id(param) not in fixed:
                l = length(correction)
                if l > maxcorr:
                    maxcorr = l
                if corrnorm > maxdelta:
                    maxdelta = corrnorm
        ...
        if maxiter and it > maxiter:
            raise SolveError(
                "failed to converge with the allowed iteration count: " + str(maxiter)
            )
