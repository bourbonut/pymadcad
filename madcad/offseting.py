...
def inflate_offsets(surface: Mesh, offset: float, method="face") -> "[vec3]":
    ...
    for i in range(5):  # the number of steps is the diffusion distance through the mesh
        for a, b in outlines:
            ...
            t = cross(
                pnormals[a] + pnormals[b], d
            )  # surface tangent normal to the edge
            # contribution stars when the offset points are closer than `offset`
            contrib = 1 - smoothstep(
                0, offset, length(offset * (pnormals[a] - pnormals[b]) + d)
            )
    ...
    if method == "face":
        ...
        return typedlist(
            (pnormals[p] * lengths[p] * offset for p in range(len(pnormals))),
            dtype=vec3,
        )
        ...
        return typedlist(
            (pnormals[p] * offset for p in range(len(pnormals))), dtype=vec3
        )
    ...
    return Mesh(
        typedlist(
            (
                p + d
                for p, d in zip(
                    surface.points, inflate_offsets(surface, offset, method)
                )
            ),
            dtype=vec3,
        ),
...
def thicken(
    surface: Mesh, thickness: float, alignment: float = 0, method="face"
) -> "Mesh":
...
def expand(surface: Mesh, offset: float, collapse=True) -> Mesh:
    ...
    for face in surface.faces:
        for e in ((face[0], face[1]), (face[1], face[2]), (face[2], face[0])):
            if e in edges:
                ...
                edges[(e[1], e[0])] = surface.facenormal(face)
    ...
    def tangent(e0, e1):
        ...
        t = normalize(
            cross(n0, n1)
            + NUMPREC * (d0 * length(d1) - d1 * length(d0))
            + NUMPREC**2 * (cross(n0, d0) + cross(n1, d1))
        )
    ...
    for loop in suites(edges, cut=False):
        ...
        for i in range(len(loop)):
            ...
            if collapse:
                ...
                for j in reversed(range(i - len(loop) + 1, i - 1)):
                    ...
                    if dot(ti - tj, pts[ei1[0]] - pts[ej0[1]]) <= NUMPREC * length2(
                        pts[ei1[0]] - pts[ej0[1]]
                    ):
                        tk += tj
                        weight += 1
        ...
        for i in range(len(extended)):
            if extended[i] != extended[i - 1]:
                mkquad(
                    surface,
                    (loop[i - 1], loop[i], j, (j if j > l else len(pts)) - 1),
                    g,
                )
                j += 1
            else:
                mktri(
                    surface, (loop[i - 1], loop[i], (j if j > l else len(pts)) - 1), g
                )
