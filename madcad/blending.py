...
def get_interface(base, tangents, weights, loops):
    ...
    if loops is None:
        ...
        if not isinstance(base, (Wire, Web, Mesh)):
            raise TypeError(
                "expected one of Wire,Web,Mesh   and not {}".format(type(base).__name__)
            )
...
def junction(
    *args, center=None, tangents="normal", weight=1.0, match="length", resolution=None
):
    ...
    if len(loops) == 0:
        return Mesh()
    if len(loops) == 1:
        return blendloop((pts, tangents, weights, loops), center, resolution=resolution)
    if len(loops) == 2:
    ...
    node = convexhull(
        [normalize(interfaces_center(pts, interf) - center) for interf in loops]
    )
    ...
    for face in node.faces:
        ...
        middle = [max(loops[i], key=lambda p: dot(pts[p], n)) for i in face]
        # middle = [None]*3
        # for i in range(3):
        ...
        for i in range(3):
            if middle[i - 1] in cuts and cuts[middle[i - 1]] != middle[i - 2]:
                continue
    ...
    for tri in middles:
        ...
        for i in range(3):
            s = tri[i]
            ptgts[i] = (
                tangents[s] * weights[s] * distance(pts[s], pts[tri[i - 2]]),
                tangents[s] * weights[s] * distance(pts[s], pts[tri[i - 1]]),
            )
            ptri[i] = pts[tri[i]]

        result += generation.dividedtriangle(
            lambda u, v: intri_smooth(ptri, ptgts, u, v), div
        )
...
def match_length(line1, line2) -> "[(int, int)]":
    ...
    while i1 < len(line1.indices) and i2 < len(line2.indices):
        p1 = (
            distance(
                line1.points[line1.indices[i1 - 1]], line1.points[line1.indices[i1]]
            )
            / l1
        )
        p2 = (
            distance(
                line2.points[line2.indices[i2 - 1]], line2.points[line2.indices[i2]]
            )
            / l2
        )
        x1p = x1 + 0.5 * p1
...
def dividematch_length(
    line1, line2, resolution=None, associated=None
) -> "[(vec3, vec3)]":
    """Insert additional couples to ensure smoothness"""
    indev


def dividematch_closest(
    w0: Wire, w1: Wire, resolution=None, associated=None
) -> (Wire, Wire):
    indev
...
def blendloop(
    interface, center=None, tangents="tangent", weight=1.0, resolution=None
) -> Mesh:
    ...
    if not center:
        center = interfaces_center(pts, *loops) + sum(
            weights[p] * tangents[p] for p in loop
        ) / len(loop)
    ...
    for i, p in enumerate(loop):
        match[i] = m = (
            (pts[p], distance(center, pts[p]) * weights[p] * tangents[p]),
            (center, noproject(pts[p] - center, normal)),
        )
        div = max(
            div,
            settings.curve_resolution(
                distance(m[0][0], m[1][0]), anglebt(m[0][1], m[1][1]), resolution
            ),
        )
...
def blendpair(
    *interfaces, match="length", tangents="tangent", weight=1.0, resolution=None
) -> Mesh:
    ...
    def infos():
        for p0, p1 in match:
            ...
            yield (
                (pts[p0], d * weights[p0] * tangents[p0]),
                (pts[p1], d * weights[p1] * tangents[p1]),
            )
...
def trijoin(pts, ptgts, div):
    ...
    for i in range(3):
        for j in range(segts):
            x = j / segts
            mesh.points.append(
                interpol2((pts[i - 1], ptgts[i - 1][0]), (pts[i], ptgts[i][1]), x)
            )
    l = len(mesh.points)
    for i in range(3):
        c = i * segts
