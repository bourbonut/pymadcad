...
def gearprofile(
    ...
    **kwargs,
    ...
    return Wire(
        points,
        tracks=tracks,
        groups=["interference", "contact", "top", "contact", "interference", "bottom"],
    )
...
def pattern_circle(
    ...
    **kwargs,
    ...
    assert circles_radius / circles_place < sin(pi / patterns), (
        "too many circles for the specified size. Change either 'ratio', 'circle_radius' or 'patterns'"
    )
    ...
    top_webs = [
        wire.transform(vec3(0, 0, half_depth - int_height)) for wire in webs_ref
    ]
    bottom_webs = [
        wire.transform(vec3(0, 0, -half_depth + int_height)).flip() for wire in webs_ref
    ]
...
def create_pattern_rect(
    ...
    **kwargs,
    ...
    bottom_webs = [
        web.transform(vec3(0, 0, -half_depth + int_height)).flip() for web in webs_ref
    ]
    ...
    **kwargs,
    ...
    if chamfer:
        if isinstance(chamfer, (tuple, list)):
            ...
            raise TypeError(
                "chamfer value must be a tuple, float or bool, not {}".format(
                    type(chamfer)
                )
            )
        if chamfer_angle < 0:
        ...
        top = wire(
            [
                ...
                radial * (rmax + margin)
                - Z * tan(chamfer_angle) * (ratio * (rmax - rmin) + margin),
            ]
        ).segmented()
    else:
        top = wire(
    ...
    if helix_angle:  # helical tooth case
        ...
        step = settings.curve_resolution(
            depth / cos(helix_angle), abs(angle), resolution
        )
        # Generation of helical gear surface
        transform = lambda x: translate(x * depth * Z) * rotate(x * angle, Z)
        tooth = extrans(
            profile, map(transform, linrange(0 - NUMPREC, 1 + NUMPREC, div=step))
        )
...
def gearstructure(
    ...
    **kwargs,
    ...
    return pattern_function(
        ext_radius, int_radius, depth, int_height, ratio=ratio, **kwargs
    )
...
def gearhub(
    ...
    **kwargs,
    ...
    if not (bore_radius):  # No hub case
        ...
        bottom_surface = circle_ref.transform(
            vec3(0, 0, -half_depth + int_height)
        ).flip()
...
def gear(
    ...
    * Extra parameters for `gearhub`
    ...
    hub = gearhub(
        ...
        depth,
        int_height,
        hub_radius=min(2 * bore_radius, 0.9 * ext_int),
        **kwargs,
    )
...
def spherical_involuteof(
    pitch_cone_angle: float, t0: float, pressure_angle: float, t: float
) -> vec3:
    """
    ...
    return +cos(pressure_angle) * spherical_involute(pitch_cone_angle, t0, t) + sin(
        pressure_angle
    ) * vec3(-cos_p * cos(t + t0), -cos_p * sin(t + t0), sin_p)
...
def jacobian_spherical_involute(
    ...
    derived_interference = lambda t: dsi(pitch_cone_angle, t02)(t) * cos(
        pressure_angle
    ) + sin(pressure_angle) * vec(t + t02)
    return lambda t1, t2: mat3(
        derived_involute(t1), -derived_interference(t2), vec3(0, 0, 1)
    )


def spherical_rack_tools(
    teeth: float, pressure_angle: float = pi / 9, ka: float = 1, kd: float = 1.25
):
    """
...
def spherical_rackprofile(
    ...
    t_min, t_max, phase1, phase2, involute = spherical_rack_tools(
        teeth, pressure_angle, ka, kd
    )
...
def spherical_gearprofile(
    ...
    n1, n2 = (
        rinvolute(t_rack_max, 0) * vec3(1, 1, 0),
        rinvolute(-t_rack_max, phase1) * vec3(1, 1, 0),
    )
    ...
    f = lambda t1, t2: involute(t1, 0) - involuteat(
        t2, -0.5 * phase_interference + beta
    )
    # Jacobian matrix
    J = jacobian_spherical_involute(
        gamma_b, gamma_p, 0, -0.5 * phase_interference + beta, pressure_angle
    )
    ...
    interference1 = [
        interference(t, -0.5 * phase_interference + beta)
        for t in linrange(0, t2, div=div)
    ]
    interference2 = [
        interference(-t, phase_diff + 0.5 * phase_interference - beta)
        for t in linrange(0, t2, div=div)
    ]
    side1 = Wire(interference1[:-1]) + Wire(
        [involute(t, 0) for t in linrange(t1, t_max, div=div)]
    )
    side2 = Wire(interference2[:-1]) + Wire(
        [involute(-t, phase_diff) for t in linrange(t1, t_max, div=div)]
    )
...
def cone_projection(profile: Wire, pitch_cone_angle: float) -> Wire:
    ...
    """
    ref = lambda t: vec3(
        sin(pitch_cone_angle) * cos(t),
        sin(pitch_cone_angle) * sin(t),
        cos(pitch_cone_angle),
    )
    new_points = [
        1 / dot(ref(atan2(point.y, point.x)), point) * point for point in profile.points
    ]
...
def straight_bevel_gear(
    ...
    spherical_profile = spherical_gearprofile(
        teeth, gamma_p, pressure_angle, ka, kd, resolution=resolution
    )  # one tooth

    # Generate teeth border
    teeth_border = extrusion(
        spherical_profile.transform(rho0 * 0.9), (rho1 * 1.1) / (rho0 * 0.9)
    )
    ...
    if bore_radius:
        if bore_height:
            ...
            G = vec3(
                1.5 * bore_radius, 0, rho1 * cos(gamma_r) + bore_height
            )  # top of the bore
            H = vec3(
                bore_radius, 0, rho1 * cos(gamma_r) + bore_height
            )  # top of the bore
    ...
    return all_teeth.transform(angleAxis(-phase, Z)).option(
        color=settings.colors["gear"]
    )
...
def helical_bevel_gear(
    ...
    def get_body_points(rho, teeth, angle):
        ...
        gear_surface_point_A = (
            helix_scale(rho - scale_step) * helix_rotate(teeth - z_step) * tooth_point
        )
        gear_surface_point_B = (
            helix_scale(rho) * transform(angleAxis(angle, Z)) * tooth_point
        )
        ...
        t_rho = _get_intersection(
            gear_surface_point_A, gear_surface_point_B, body_C, body_D
        )
    ...
    t_max = acos(cos(gamma_f) / cos_b) / sin_b
    middle_tooth = 0.5 * (involute(t_max, 0) + involute(-t_max, phase_diff))
    return all_teeth.transform(quat(X, middle_tooth * v)).option(
        color=settings.colors["gear"]
    )
