...
display = {
    ...
    "sharp_angle": pi
    / 6,  # above this threshold angles between mesh triangles are considered sharp, below they are rendered softened
    ...
    "solid_color_front": 1.0,  # surface intensity for portions of surface parallel to the view direction
...
resolution = [
    "rad",
    pi / 16,
]  # maximum angle for discretizing curves, see `curve_resolution()`
    ...
    def update(dst, src):
        for key in dst:
            if key in src:
                ...
                elif isinstance(dst[key], fvec3):
                    dst[key] = fvec3(src[key])
                elif isinstance(dst[key], fvec4):
                    dst[key] = fvec4(src[key])
                else:
                    dst[key] = src[key]
...
def dump(file=None):
    ...
    yaml.add_representer(
        fvec3, lambda dumper, data: dumper.represent_list(round(f, 3) for f in data)
    )
    yaml.add_representer(
        fvec4, lambda dumper, data: dumper.represent_list(round(f, 3) for f in data)
    )
...
def use_qt_colors():
    ...
    hover = mix(fvec3(0.0, 0.4, 1), qtc(role.Highlight), 0.6)
    ...
    colors.update(
        {
            ...
            "schematic": mix(
                qtc(role.Text) * normalize(qtc(role.LinkVisited) + 0.01),
                qtc(role.LinkVisited),
                0.5,
            ),
            "annotation": mix(
                qtc(role.Text) * normalize(qtc(role.Link) + 0.01), qtc(role.Link), 0.5
            ),
        }
    )
