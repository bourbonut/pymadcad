...
def _next_separator(text, seek):
    ...
    for i in range(seek, len(text)):
        if text[i] in begin_marker:
            counter += 1
        elif text[i] in end_marker:
            counter -= 1
        if counter < 0 or (counter == 0 and text[i] in separators):
            break
    return i
...
def nformat(text, indent=0, width=None):
    ...
    while seek < len(text):
        if text[seek] in begin_marker:
            ...
            if (
                term - seek > width or "\n" in text[seek:term]
            ):  # develop enclosement if its length is over that width
                indent += 1
                out.write(text[seek])
                if (
                    _next_separator(text, seek + 1) < term
                ):  # no return if there is only one element in the enclosement
                    out.write("\n" + "\t" * indent)
    ...
    while seek < len(text):
        if text[seek] in spaces:
            ...
            if text[seek - 1] in ",+-*/":
                ...
                text[seek - 1] in begin_marker
                or jump < len(text)
                and text[jump] in end_marker
            ):
                pass
            elif "\n" in text[seek:jump]:
                out.write("\n")
    ...
    for symbol in chain(separators, begin_marker, end_marker, symbols, specials):
        ...
        while start != -1:
            ...
            if symbol == "<":
                ...
                color = color_specials

            coloration.insert(
                _bisect(coloration, start, lambda x: x[1]), (color, start, end - 1)
            )
    ...
    for c in it:
        if c.isalnum():
            ...
            if c.isdigit():
                ...
                j += 1
                coloration.insert(
                    _bisect(coloration, i, lambda x: x[1]), (color_number, i, i + j)
                )
                i += j + 1
...
if __name__ == "__main__":
    ...
    nprint(
        "here is a list:",
        [[1000, 2324, 30], [40000, 5000, 6342342]],
        "\nhere is a type:",
        int,
    ...
    nprint(
        "hello everyone, 100 is a decimal number and (0x27a) is an hexadecimal one between parentheses. (did you noticed the automatic line shift ?)"
    )
