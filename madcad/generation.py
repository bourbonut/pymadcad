...
def revolution(
    shape, axis=Axis(O, Z), angle: float = 2 * pi, alignment: float = 0, resolution=None
) -> Mesh:
...
def helix(
    ...
    return extrans(
        shape,
        (
            translate(t * height * axis[1])
            * rotatearound(t * height * helix / radius, axis)
            for t in linrange(0 - alignment, 1 - alignment, div=div)
        ),
    )
...
def tube(shape, path: Wire, end=True, section=True) -> Mesh:
    ...
    if path.tracks:
        links = ((i, i + 1, path.tracks[i]) for i in range(len(path) - 1))
    else:
        links = ((i, i + 1, 0) for i in range(len(path) - 1))
    # generate
...
def extrans(section, transformations: iter, links=None) -> Mesh:
    ...
    for (u, v), t in groups.items():
        mesh.groups[t] = section.groups[
            v
        ]  # NOTE will change in the future to mention both u and v
...
def icosurface(pts, ptangents, resolution=None) -> "Mesh":
    ...
    if isinstance(ptangents[0], tuple):
        ...
        for i in range(3):
            ptangents[i] = (
                normalize(noproject(pts[i - 2] - pts[i], normals[i]))
                * arclength(pts[i], pts[i - 2], normals[i], normals[i - 2]),
                normalize(noproject(pts[i - 1] - pts[i], normals[i]))
                * arclength(pts[i], pts[i - 1], normals[i], normals[i - 1]),
            )
...
def parallelogram(*directions, origin=vec3(0), align=vec3(0), fill=True) -> "Mesh":
    ...
    for i in range(2 ** len(directions)):
        points.append(
            min + sum(d if i >> k & 1 else 0 for k, d in enumerate(directions))
        )
...
def cylinder(
    bottom: vec3, top: vec3, radius: float, fill=True, resolution=None
) -> "Mesh":
    ...
    base = wire(
        primitives.Circle(Axis(bottom, normalize(direction)), radius),
        resolution=resolution,
    )
    if fill:
        base = flatsurface(base).flip()
...
def cone(summit: vec3, base: vec3, radius: float, fill=True, resolution=None) -> "Mesh":
    ...
    """
    base = wire(
        primitives.Circle(Axis(base, normalize(summit - base)), radius),
        resolution=resolution,
    )
    if fill:
        base = flatsurface(base)
...
def square(axis: primitives.Axis, width: float) -> "Mesh":
    ...
    return Mesh(
        typedlist(
            [axis[0] + width * p for p in ((x + y), (y - x), (-y - x), (-y + x))]
        ),
...
def repeataround(pattern, repetitions: int = None, axis=Axis(O, Z), angle=2 * pi):
    ...
    if repetitions is None:
        if isinstance(pattern, Mesh):
            indices = (i for f in pattern.faces for i in f)
        elif isinstance(pattern, Web):
            indices = (i for e in pattern.edges for i in e)
        elif isinstance(pattern, Wire):
            indices = iter(pattern.indices)

        x, y, z = dirbase(axis[1], align=pattern.points[next(indices)] - axis[0])
        lower, upper = 0, 0
        for p in indices:
            t = atan2(
                dot(pattern.points[p] - axis[0], y), dot(pattern.points[p] - axis[0], x)
            )
            lower = min(lower, t)
            upper = max(upper, t)
