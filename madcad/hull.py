...
def simple_convexhull(points: "[vec3]") -> "[uvec3]":
    ...
    """
    return numpy_to_typedlist(
        scipy.spatial.ConvexHull(
            typedlist_to_numpy(points, "f8"), qhull_options="QJ Pp"
        ).simplices,
        uvec3,
    )
...
def convexoutline(source: "[vec3]", normal: vec3 = None, flatten: bool = False) -> Web:
    ...
    if flatten:
        ...
        outline.points = typedlist(
            center + noproject(p - center, direction) for p in outline.points
        )
    return outline
...
def restore_groups(source, indices) -> Mesh:
    ...
    if isinstance(source, Mesh):
        former = {
            facekeyo(*face): track for face, track in zip(source.faces, source.tracks)
        }
    elif isinstance(source, Web):
        former = {edge: track for edge, track in zip(source.edges, source.tracks)}
    else:
...
def widest_surface_direction(mesh) -> vec3:
    ...
    for face in directionmap:
        surf = length2(
            cross(
                normals[face[1]] - normals[face[0]], normals[face[2]] - normals[face[0]]
            )
        )
        if surf >= score:
    ...
    for face in mesh.faces:
        surf = cross(
            mesh.points[face[1]] - mesh.points[face[0]],
            mesh.points[face[2]] - mesh.points[face[0]],
        )
        if dot(surf, proj) > 0:
            direction += surf
