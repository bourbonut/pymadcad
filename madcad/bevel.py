...
def planeoffsets(mesh, edges, **cutter):
    ...
    for f in mesh.faces:
        for e in ((f[0], f[1]), (f[1], f[2]), (f[2], f[0])):
            if e in offsets:
                offsets[e][0] = mesh.facenormal(f)
            elif (e[1], e[0]) in offsets:
                offsets[(e[1], e[0])][1] = mesh.facenormal(f)
...
def mesh_cut(mesh, start, cutplane, stops, conn, prec, removal, cutghost=True):
    ...
    if isinstance(start, tuple):
        ...
        raise TypeError(
            "wrong start: {}, cut can only start from a point or an edge".format(start)
        )
    intersections = set()
    seen = set()
    while front:
        ...
        if not any(goodside):
            continue
        outside = any(
            all(dot(pts[pi] - stop[0], stop[1]) <= -prec for pi in f) for stop in stops
        )
        if outside:
            continue
        ...
        if (
            fi not in removal or cutghost
        ):  # NOTE: this is a ugly trick to make the current edgecut implementation work with the same function for corners and edges
            # cut the face
            for j in range(3):
                if cut[j] and cut[j - 1]:
                    # cut only if the intersection segment is in the delimited area
                    if any(
                        dot(cut[j - 1] - stop[0], stop[1]) <= prec
...
def mesh_edgecut(mesh, edges, conn=None, prec=None, removal=None, **cutter):
    ...
    for i, prox in juncconn.items():
        ...
        axis = intersection_plane_plane(
            (pts[prox[0]] + o0, o0), (pts[prox[1]] + o1, o1), pts[i]
        )  # fail when the two planes have the same orientation
        if axis:
            n = normalize(cross(pts[i] - axis[0], axis[1]))
            if dot(n, pts[prox[0]] - pts[i]) < 0:
                n = -n
    ...
    for e, sep in separators.items():
        ...
        while not sep:
            prox = juncconn[i]
            if len(prox) != 2:
                raise MeshError("unable to compute all separators")
...
def intersection_edge_plane(edge, axis, prec):
    ...
    if abs(db) <= prec:
        return b
    if da * db > 0:
        return None  # the segment doesn't reach the plane
    edgedir = b - a
    proj = dot(edgedir, axis[1])
    if abs(proj) <= prec:
        return None  # the segment is parallel to the plane
    ...
    for e, s in segments.items():
        ...
        # edge cuts
            ...
            if len(frags) == 1:
                ...
                l, r = frags
                if (
                    dot(pts[l[-1]] - pts[l[0]], pts[e[1]] - pts[e[0]]) < 0
                    or dot(pts[r[-1]] - pts[r[0]], pts[e[1]] - pts[e[0]]) > 0
                ):
                    l, r = r, l
...
def mesh_filet(mesh, edges, resolution=None, **cutter):
    ...
    for e, s in segments.items():
        ...
        # edge cuts
            ...
            if len(frags) == 1:
                ...
                side = noproject(
                    pts[lp[0]] - pts[lp[-1]], normalize(pts[e[1]] - pts[e[0]])
                )
                for i in range(1, len(lp)):
                    if (
                        dot(pts[lp[i - 1]] - pts[e[0]], side)
                        * dot(pts[lp[i]] - pts[e[0]], side)
                        < 0
                    ):
                        l, r = lp[:i], lp[i:]
                        break
            ...
            for i in range(len(match)):
                if i:
                    x += distance(pts[match[i - 1][0]], pts[match[i][0]]) / ll
    ...
    for e, s in corners.items():
        ...
        lp = suites(s)[0]
        if lp[-1] == lp[0]:
            lp.pop()  # remove the redundant point if there is (not all the time due to some BUG)
...
def web_chamfer(web, points, **cutter):
    ...
    def link(e):
        if web.edges[next(conn[e[0]])][0] == e[0]:
            return (e[1], e[0])
        else:
            return e
...
def web_filet(obj, points, resolution=None, **cutter):
    ...
    def tangentat(p):
        ...
        if e[0] == p:
            return pts[p], normalize(pts[p] - pts[e[1]]), True
        else:
            return pts[p], normalize(pts[p] - pts[e[0]]), False
...
def wire_edgecut(wire: Wire, points, **cutter):
    ...
    for index, origin in enumerate(wire.indices):
        ...
        cuts.append(
            (iiback, iiback + 1)
        )  # point interval including start and excluding end
