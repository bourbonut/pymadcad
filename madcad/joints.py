...
class Planar(Joint):
    ...
    def transmitable(self, force, parameters=None, velocity=None):
        ...
        return Screw(
            project(action.resulting, normal),
    ...
    def scheme(self, index, maxsize, attach_start, attach_end):
        ...
        square = gt.parallelogram(
            x * size, y * size, origin=o - gap * size * z, align=0.5
        )
...
class Prismatic(Joint):
    ...
    def transmit(self, force, parameters=None, velocity=None):
        ...
        return Screw(
            noproject(action.resulting, normal), action.momentum, action.position
        )
    ...
    def scheme(self, index, maxsize, attach_start, attach_end):
        ...
        profile = gt.parallelogram(
            0.4 * size * x, 0.4 * size * y, origin=o, align=0.5, fill=False
        )
...
class Cylindrical(Joint):
    ...
    def direct(self, parameters) -> mat4:
        return (
            self.post
            * rotate(parameters[0], Z)
            * translate(parameters[1] * Z)
            * self.pre
        )
...
class Ball(Joint):
    ...
    def scheme(self, index, maxsize, attach_start, attach_end):
        ...
        if attach_start:
            ...
            x, y, z = mat3()
        profile = ArcCentered(Axis(O, y), x * radius, -z * radius).mesh(
            resolution=resolution
        )
...
class PointSlider(Joint):
    ...
    def scheme(self, index, maxsize, attach_start, attach_end):
        ...
        if attach_start:
            attach = o + radius * z
            sch.add(
                [attach, attach + z * 0.1 * size, attach + (x + y) * 0.1 * size],
                shader="fill",
            )
        ...
        if attach_end:
            ...
            sch.add(
                [
                    attach,
                    attach + z * cornersize * size,
                    attach + (x + y) * cornersize * size,
                ],
                shader="fill",
            )
            sch.add([attach + cornersize * size * z, attach_start], shader="line")
    ...
    def scheme(self, index, maxsize, attach_start, attach_end):
        ...
        if attach_start:
            ...
            sch.add(
                [
                    attach,
                    attach + cornersize * size * z,
                    attach + cornersize * size * (x + y),
                ],
                shader="fill",
            )
...
class Ring(Joint):
    ...
    def scheme(self, index, maxsize, attach_start, attach_end):
        ...
        if attach_start:
            ...
            x, y, z = mat3()
        profile = ArcCentered(
            Axis(center, y), center + x * radius, center - z * radius
...
class Universal(Joint):
    ...
    def scheme(self, index, maxsize, attach_start, attach_end):
        ...
        if attach_start:
            ...
            sch.add(o - z * radius)
        sch.add(
            ArcCentered(Axis(o, x), y * radius, -y * radius).mesh(resolution),
            shader="line",
        )
        sch.add([y, -y])
        ...
        if attach_end:
            ...
            sch.add(o - z * radius)
        sch.add(
            ArcCentered(Axis(o, y), x * radius, -x * radius).mesh(resolution),
            shader="line",
        )
        sch.add([x, -x])
...
class Rack(Joint):
    ...
    def direct(self, translation):
        return (
            ...
            * rotate(-translation / self.radius, Z)
            * self.pre
        )
    ...
    def grad(self, translation):
        return (
            ...
            * self.pre
            + self.post
            * translate(translation * X)
            * drotate(-translation / self.radius, Z)
            / -self.radius
            * self.pre
        )
    ...
    def scheme(self, index, maxsize, attach_start, attach_end):
        ...
        sch.add(
            revolution(
                Segment(o + self.radius * x + h, o + self.radius * x - h), Axis(o, z)
            ),
            shader="ghost",
        )
...
class Gear(Joint):
    ...
    def scheme(self, index, maxsize, attach_start, attach_end):
        ...
        sch.add(
            revolution(Segment(o + r0 * x + h, o + r0 * x - h), Axis(o, z)),
            shader="ghost",
        )
        ...
        sch.add(
            revolution(Segment(o + r1 * x + h, o + r1 * x - h), Axis(o, z)),
            shader="ghost",
        )
...
class Helicoid(Joint):
    ...
    def __init__(self, s0, s1, step, b0, b1=None, position=None):
        ...
        self.solids = s0, s1
        if isaxis(b0):
            b0 = b0[0], *dirbase(b0[1])[:2]
        if b1 and isaxis(b1):
            b1 = b1[0], *dirbase(b1[1])[:2]
    ...
    def scheme(self, solid, size, junc):
        if solid is self.solids[0]:
            ...
            for i in range(-div, div + 1):
                t = i / div
                heli.append(
                    radius * (cos(a * t) * x + sin(a * t) * y) + center + side * t
                )
