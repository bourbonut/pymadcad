    ...
    for i in range(len(m1.faces)):
        ...
        if grp[i] == -1 and m1.facepoints(i) in prox2:
            ...
            while front:
                ...
                if (
                    grp[fi] == -1
                    and m1.tracks[fi] == track
                    and distance2(m1.facenormal(fi), normal) <= NUMPREC
                ):
                    surf.append(fi)
                    grp[fi] = currentgrp
                    f = m1.faces[fi]
            ...
            for f1 in surf:
                ...
                for f2 in set(prox2.get(m1.facepoints(f1))):
                    intersect = core.intersect_triangles(
                        m1.facepoints(f1), m2.facepoints(f2), 8 * prec
                    )
                    if intersect:
                        ia, ib = intersect[0][2], intersect[1][2]
                        if distance2(ia, ib) <= prec**2:
                            continue
                        ...
                        for i in range(2):
                            ...
                            e = min(
                                outline,
                                key=lambda e: distance_pe(
                                    p, (m1.points[e[0]], m1.points[e[1]])
                                ),
                            )  # this perfect minimul should not be needed as the first below the precision should be unique, but for precision robustness ...
                            if seg[i] not in e:
                                # print('  cross', i, e)
                                outline.remove(e)
        ...
        for edge in ((e[0], e[1]), (e[1], e[0])):
            if edge in conn1:
                ...
                for i in range(3):
                    if f[i] not in edge:
                        proj = dot(
                            m1.points[f[i]] - m1.points[f[i - 1]], m2.facenormal(f2)
                        ) * (-1 if side else 1)
                        if proj > prec:
                            used[fi] = 1
                            conn1[(f[i - 1], f[i])] = fi
    ...
    while front:
        ...
        for edge in front:
            if edge in conn1:
                ...
                if not used[fi] or (
                    used[source] * used[fi] < 0 and abs(used[fi]) > abs(used[source])
                ):
                    assert not (strict and used[fi]), (
                        "the pierced surface is both side of the piercing surface"
                    )
    ...
    for e1 in range(len(w1.edges)):
        ...
        if close:
            ...
            if segts:
                ...
                sorted_segts = sorted(
                    segts.items(), key=lambda s: dot(w1.points[s[0]], direction)
                )
...
def pierce_web(web, ref, side=False, prec=None) -> Web:
    ...
    order = sorted(
        range(len(web.points)), key=lambda i: distance2(web.points[i], center)
    )
...
def boolean_web(w1, w2, sides, prec=None) -> Web:
    if not prec:
        prec = max(w1.precision(), w2.precision())
    ...
    for p2, ei in zip(frontier.indices, frontier.tracks):
        ...
        direction = (
            ...
            ^ sides[1]
            ^ (
                distance2(w2.points[p2], mc1.points[e1[0]])
                < distance2(w2.points[p2], mc1.points[e1[1]])
            )
        )
        for ei in conn2[p2]:
...
def intersect_edges(e1: "(vec3,vec3)", e2: "(vec3,vec3)", prec) -> vec3:
    ...
    if dot(e1[0] - p, d1) > prec or dot(p - e1[1], d1) > prec:
        return
    if dot(e2[0] - p, d2) > prec or dot(p - e2[1], d2) > prec:
        return
    return p
...
def cut_web_mesh(w1: Web, ref: Mesh, prec=None) -> "(Web, Wire)":
    ...
    for e1 in range(len(w1.edges)):
        ...
        if close:
            ...
            segts = {}
            for f2 in close:
                intersect = intersect_edge_face(
                    w1.edgepoints(e1), ref.facepoints(f2), prec
                )
                if intersect:
                    seg = points.add(intersect)
...
def pierce(m, ref, side=False, prec=None):
    ...
    if not op:
        raise TypeError(
            "pierce is not possible between {} and {}".format(
                type(m).__name__, type(ref).__name__
            )
        )
...
def boolean(a, b, sides=(False, True), prec=None):
    ...
    if not op:
        raise TypeError(
            "boolean is not possible between {} and {}".format(type(a), type(b))
        )
