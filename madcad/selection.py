...
def select(mesh, edge, stopleft=None, stopright=False, conn=None, web=None) -> Web:
    ...
    if stopleft is None:
        stopleft = lambda *args: False
    if stopright is None:
        stopright = lambda *args: False
    elif stopright is False:
        stopright = stopleft
...
def selectside(shared, edge, stop, seen=None, revert=None):
    ...
    while front:
        last, curr = front.pop()
        if uvec2(last, curr) in seen:
            continue
        elif uvec2(curr, last) in seen:
            continue
        if revert:
            seen.add(uvec2(curr, last))
        else:
            seen.add(uvec2(last, curr))
        connected = conn[curr]
        if len(connected) > 1:
...
class selexpr(object):
    ...
    def __and__(self, other):
        return selexpr(lambda *args: self.func(*args) and other.func(*args))

    def __or__(self, other):
        return selexpr(lambda *args: self.func(*args) or other.func(*args))

    def __xor__(self, other):
        return selexpr(lambda *args: bool(self.func(*args) ^ other.func(*args)))

    def __not__(self, other):
        return selexpr(lambda *args: not self.func(*args))
...
def stopangle(maxangle):
    ...
    def stop(shared, last, curr, next):
        ...
        return (
            anglebt(points[curr] - points[last], points[next] - points[curr])
            >= maxangle
        )
...
def straight(shared, last, curr, next):
    ...
    other = min(
        shared["conn"][curr], key=lambda o: anglebt(start, points[o] - points[curr])
    )
    return other != next
...
def edgenear(web, obj):
    if isinstance(obj, vec3):
        dist = lambda e: distance_pe(obj, e)
    elif isinstance(obj, tuple):
        dist = lambda e: distance_ae(obj, e)
    else:
        raise TypeError("obj must be a point or an axis")
    if isinstance(web, Mesh):
        web = web.groupoutlines()
    best = None
    score = math.inf
    ...
    if (
        ...
        and dot(web.points[best[1]] - web.points[best[0]], obj[1]) < 0
    ):
        best = best[1], best[0]
    return best
