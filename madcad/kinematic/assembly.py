...
class Solid(dict):
    ...
    def append(self, value) -> int:
        ...
        key = next(i for i in range(len(self.content) + 1) if i not in self.content)
        self[key] = value
        return key
...
def placement(*pairs, precision=1e-3):
    ...
    for pair in pairs:
        if len(pair) == 2:
            joints.append(guessjoint((0, 1), *pair, precision * 0.25))
        elif len(pair) == 3:
            joints.append(pair[0]((0, 1), *pair[1:]))
        else:
...
def explode_offsets(solids) -> "[(solid_index, parent_index, offset, barycenter)]":
    ...
    def process(i, solid):
        if hasattr(solid.content, "values"):
            it = solid.content.values()
        else:
            it = solid.content
        for obj in solid.content.values():
            if isinstance(obj, Solid):
                ...
                try:
                    points[i].extend(
                        extract_used(
                            convexhull(extract_used(obj)).transform(solid.pose)
                        )
                    )
        ...
        for j in range(len(solids)):
            ...
            if d < scores[i]:
                ...
                if isinstance(prim, int):
                    ...
                    offsets[i] = noproject(
                        center - pts[prim[0]], pts[prim[0]] - pts[prim[1]]
                    )
                elif len(prim) == 3:
                    normal = cross(
                        pts[prim[1]] - pts[prim[0]], pts[prim[2]] - pts[prim[0]]
                    )
                    offsets[i] = project(center - pts[prim[0]], normal)
                else:
                    raise AssertionError(
                        "prim should be an index for  point, face, triangle"
                    )
...
def explode(solids, factor=1, offsets=None) -> "(solids:list, graph:Mesh)":
    ...
    shifts = [
        (solids[solid].position - solids[parent].position) if parent else vec3(0)
        for solid, parent, offset, center in offsets
    ]
    for solid, parent, offset, center in offsets:
        if parent:
            solids[solid].position = (
                solids[parent].position + shifts[solid] + offset * factor
            )
