...
class SolidDisplay(Group):
    ...
    def box(self):
        return boundingbox(
            ...
            ignore=True,
        ).transform(self._local * self._free)

    @writeproperty
    def local(self):
        self._place_displays()
    ...
    def stack(self, scene):
        ...
        for key, display in self.displays.items():
            ...
            if (
                key == "annotations"
                and not scene.options["display_annotations"]
    ...
    def _move(self, view, evt):
        ...
        moving = [
            (solid, solid.free)
            for solid in view.scene.selection
            if isinstance(solid, SolidDisplay)
        ]
        ...
        while True:
            ...
            for solid, former in moving:
                move = transpose(fmat3(solid._world * solid._local * solid._free)) * (
                    click - anchor
                )
...
class ChainManip(Group):
    ...
    def control(self, view, key, sub, evt):
        ...
        if (
            evt.type() == QEvent.Type.MouseMove
            ...
            if sub == ("scheme", index_toolcenter):
                ...
                generator = getattr(
                    self, "move_" + view.scene.options["kinematic_manipulation"]
                )(view, sub, evt)
    ...
    def move_tool(self, view, sub, evt):
        ...
        while True:
            ...
            self.toolcenter = affineInverse(place) * (
                view.ptfrom(evt.pos(), init) + offset
            )
    ...
    def move_joint(self, view, sub, evt):
        ...
        joint = max(0, solid - 1)
        anchor = affineInverse(mat4(self.world) * self.parts[joint + 1]) * vec4(
            click, 1
        )
        ...
        while True:
            ...
            if evt.type() in (QEvent.Type.MouseMove, QEvent.Type.MouseButtonRelease):
                ...
                model = (
                    mat4(view.uniforms["proj"] * view.uniforms["view"] * self.world)
                    * self.parts[joint]
                )
                ...
                jac = np.stack(
                    [(model * (grad * anchor)).xy / current.w for grad in jac]
                )
                ...
                colinearity = min(
                    ...
                    sum(
                        dot(grad, move) ** 2
                        / (length2(grad) + length2(move) + self.prec)
                        for grad in jac
                    )
                    / self.min_colinearity,
                )
                # combination of the closest directions to the mouse position
                increment = la.solve(
                    jac @ jac.transpose() + np.eye(len(jac)) * self.prec,
                    jac @ (move * colinearity),
                )
                self.pose[joint] = self.chain.joints[joint].normalize(
                    (
                        np.asarray(self.pose[joint])
                        + increment
                        * min(1, self.max_increment / np.abs(increment).max())
    ...
    def move(self, move, jac):
        ...
        self.pose = self.chain.normalize(
            structure_state(
                (
                ...
                self.move(
                    move=np.concatenate(
                        [
                            ...
                            - (model * solid * normalize(anchor - tool)).xy
                            / current_tool.w,
                        ]
                    ),
                    jac=np.stack(
                        [
                            np.concatenate(
                                [
                                    (grad * tool).xyz,
                                    (model * grad * normalize(anchor - tool)).xy
                                    / current_tool.w,
                                ]
                            )
                            for grad in jac
...
class KinematicManip(Group):
    ...
    def __init__(self, scene, kinematic, pose=None, toolcenter=None):
        ...
        try:
            ...
            warnings.warn(
                "could not solve kinematic {} for displaying, falling back to infinite tolerance".format(
                    self
                )
            )
    ...
    def place_solids(self, view):
        ...
        for space in self.displays["scheme"].spacegens:
            if (
                isinstance(space, (world_solid, scale_solid))
                and space.solid in self.parts
            ):
    ...
    def move_tool(self, view, sub, evt):
        ...
        while True:
            ...
            self.toolcenter = affineInverse(place) * (
                vec3(view.ptfrom(evt.pos(), init)) + offset
            )
    ...
    def move_opt(self, optmove, optjac):
        ...
        priority = self.move_precision / max(
            self.move_precision, (costmove**2).max(initial=0)
        )
        # colinearity factor, lowering the optional move while it doesn't match the mechanism degrees of freedom
        colinearity = min(
            1,
            max(
                np.dot(optgrad, optmove) ** 2
                / (normsq(optgrad) * normsq(optmove) + self.prec**4)
        ...
        increment = la.solve(
            jac @ jac.transpose() + np.eye(len(jac)) * self.prec, jac @ move
        )
        # assemble the new pose and normalize it
        newpose = self.kinematic.normalize(
            structure_state(
                flatten_state(self.pose)
                + increment * min(1, self.max_increment / np.abs(increment).max()),
                self.pose,
        ...
        # try to move
        try:
            self.parts = self.kinematic.parts(
                self.pose, precision=self.tolerated_precision
            )
            self.pose = newpose
        except KinematicError:
            pass
...
class scale_solid:
    ...
    def __call__(self, view):
        ...
        return (
            ...
            * scale(fvec3(min(self.size, 2 / (e[1] * view.target.height))))
        )
...
def kinematic_toolcenter(toolcenter):
    ...
    sch.add(
        [size * cos(t) * X + size * sin(t) * Y for t in linrange(0, 2 * pi, step=0.2)],
        color=fvec4(color, 1),
    )
    sch.add(
        [
            radius * cos(t) * X + radius * sin(t) * Y
            for t in linrange(0, angle, step=0.1)
        ],
        color=fvec4(color, 1),
    )
    sch.add(
    ...
    sch.add(
        revolution(
            web([radius * rend, radius * rend - 14 * tend - 3.6 * rend]),
            Axis(radius * rend, tend),
            resolution=("div", 8),
        ),
        shader="fill",
...
def kinematic_scheme(joints) -> "(Scheme, index)":
    ...
    for joint in joints:
        if hasattr(joint, "position"):
            ...
            sch = joint.scheme(
                index, size, centers[joint.solids[0]], centers[joint.solids[-1]]
            )
