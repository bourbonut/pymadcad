...
__all__ = [
    ...
    "screw_slot",
    "bolt_slot",
    "bearing_slot_exterior",
    "bearing_slot_interior",
    "circular_screwing",
    "grooves_profile",
    "grooves",
    "stfloor",
...
standard_digits = [0.0, 1.0, 0.5, 0.4, 0.6, 0.8, 0.2, 0.25, 0.75]
...
def screw(d, length, filet_length=None, head="SH", drive=None, detail=False):
    ...
    """
    if filet_length is None:
        filet_length = length - 0.05 * d
    elif length < filet_length:
        raise ValueError("filet_length must be smaller than length")

    head, drive = screw_spec(head, drive)
    head = globals()["screwhead_" + head](d)
    if drive:
        drive = globals()["screwdrive_" + drive](d).transform(
            boundingbox(head).max[2] * Z
        )
...
def screwhead_button(d):
    ...
    profile = [
        ...
        TangentEllipsis(vec3(0, 0.5 * d, h), vec3(0, 0.9 * r, h), vec3(0, r, 0.1 * d)),
        wire(
            [
                vec3(0, r, 0.1 * d),
                vec3(0, r, 0),
...
def washer(d, e=None, h=None) -> Mesh:
    ...
    return Solid(
        part=thicken(revolution(web([e / 2 * X, d / 2 * X])), h).option(
            color=settings.colors["bolt"]
        ),
...
def section_s(height=1, width=None, flange=None, thickness=None) -> Web:
    ...
    base = wire(
        [
            ...
            vec3(
                thickness / 2,
                height / 2 - flange - 0.1 * (width / 2 - thickness / 2),
                0,
            ),
        ]
    ).flip()
    ...
    filet(
        section,
        section.frontiers(0, 5, 10, 4, 6, 11),
        radius=thickness * 0.4,
        resolution=("div", 2),
    )
    filet(
        section,
        section.frontiers(1, 0, 4, 3, 6, 7, 10, 9),
        radius=flange,
        resolution=("div", 2),
    )
...
def section_c(height=1, width=None, thickness=None) -> Web:
    ...
    filet(
        section,
        section.frontiers(0, 1, 5, 7, 6),
        radius=0.8 * thickness,
        resolution=("div", 2),
    )
    return section.finish()
...
def section_tslot(size=1, slot=None, thickness=None, depth=None) -> Web:
    ...
    base = (
        base + base.transform(scaledir(normalize(vec3(1, -1, 0)), -1)).flip()
    ).segmented()
    base = base + base.transform(scaledir(X, -1)).flip()
    base = base + base.transform(scaledir(Y, -1)).flip()
    section = web(
    ...
    filet(
        section,
        section.frontiers(5, 7, 41, 43, 31, 29, 17, 19),
        radius=thickness / 2,
        resolution=("div", 2),
    )
...
def coilspring_compression(length, d=None, thickness=None, solid=True):
    ...
    return Solid(
        part=tube(
            flatsurface(
                Circle((path[0], Y), thickness / 2, resolution=("div", 6))
            ).flip(),
            path,
        ),
...
def coilspring_tension(length, d=None, thickness=None, solid=True):
    ...
    coil = Wire(
        [
            vec3(r * cos(t), r * sin(t), z0 + t / (2 * pi) * thickness)
    ...
    return Solid(
        part=tube(
            flatsurface(Circle((path[0], Z), thickness / 2, resolution=("div", 6))),
            path,
        ),
        axis=Axis(O, Z, interval=(-length / 2, length / 2)),
        top=0.5 * length * Z,
...
@cachefunc
def coilspring_torsion(
    arm, angle=radians(45), d=None, length=None, thickness=None, hook=None, solid=True
) -> Solid:
    ...
    coil = Wire(
        [
            vec3(-r * sin(t), r * cos(t), z0 + t / (2 * pi) * thickness)
            for t in linrange(0, 2 * pi * ncoil, step)
        ]
    ).qualify("spring")
...
def bearing_ball(dint, dext=None, h=None, sealing=False, detail=False) -> Solid:
    ...
    if detail and not sealing:
        ...
        interior += wire(
            ArcCentered((rb * X, -Y), vec3(rint + e, 0, hr), vec3(rint + e, 0, -hr))
        )
        exterior += wire(
            ArcCentered((rb * X, -Y), vec3(rext - e, 0, -hr), vec3(rext - e, 0, hr))
        )
        interior.close()
        exterior.close()
        part = revolution(web([exterior, interior]), axis).option(
            color=settings.colors["bearing"]
        )

        nb = int(0.7 * pi * rb / rr)  # number of balls that can fit in
        balls = repeat(
            icosphere(rb * X, rr), nb, angleAxis(radians(360) / nb, Z)
        ).option(color=vec3(0, 0.1, 0.2))
        ...
        surf = extrusion(
            cage_profile,
            mat3((rb - rr * 0.6) / rb, (rb - rr * 0.6) / rb, 1),
            alignment=0.5,
        )

        cage = thicken(surf + surf.transform(mat3(1, 1, -1)).flip(), c).option(
            color=settings.colors["bearing_cage"]
        )
        ...
        interior = (
            Wire(
                [exterior[-1], exterior[-1] + c * Z, interior[0] + c * Z, interior[0]]
            ).segmented()
            + interior
            + Wire(
                [interior[-1], interior[-1] - c * Z, exterior[0] - c * Z, exterior[0]]
            ).segmented()
        )
        return Solid(
            part=revolution(web([exterior, interior]), axis).option(
...
def bearing_roller(
    dint, dext=None, h=None, contact=0, hint=None, hext=None, detail=False
) -> Solid:
...
def bearing_thrust(dint, dext, h, detail=False) -> Solid:
    ...
    if detail:
        ...
        top += wire(
            ArcCentered((rb * X, -Y), vec3(rb + hr, 0, w - e), vec3(rb - hr, 0, w - e))
        )
        bot += wire(
            ArcCentered(
                (rb * X, -Y), vec3(rb - hr, 0, -w + e), vec3(rb + hr, 0, -w + e)
            )
        )
        top.close()
        bot.close()
        part = revolution(web([top, bot]), axis).option(
            color=settings.colors["bearing"]
        )
        ...
        top = (
            Wire([bot[-1], bot[-1] + c * X, top[0] + c * X, top[0]]).segmented()
            + top
            + Wire([top[-1], top[-1] - c * X, bot[0] - c * X, bot[0]]).segmented()
        )
...
def slidebearing(dint, h=None, thickness=None, shoulder=None, opened=False) -> Solid:
    ...
    line = select(part, vec3(-rint, 0, -h), stopangle(pi / 2)) + select(
        part, vec3(dint, dint, -h), stopangle(pi / 2)
    )
    if not shoulder:
        line += select(part, vec3(-rint, 0, 0), stopangle(pi / 2)) + select(
            part, vec3(dint, dint, 0), stopangle(pi / 2)
        )
...
def bolt(
    a: vec3, b: vec3, dscrew: float, washera=False, washerb=False, nutb=True
) -> Solid:
    ...
    result = Solid(
        screw=rscrew.place(
            (Revolute, rscrew["axis"], Axis(a - thickness * dir * int(washera), -dir))
        ),
    )
    if washera:
    ...
    if nutb:
        rnut = nut(dscrew)
        result["nut"] = rnut.place(
            (Revolute, rnut["top"], Axis(b + thickness * dir * int(washerb), -dir))
        )
    return result


def screw_slot(
    axis: Axis, dscrew: float, rslot=None, hole=0.0, screw=0.0, expand=True, flat=False
) -> Mesh:
...
def bolt_slot(
    a: vec3, b: vec3, dscrew: float, rslot=None, hole=True, expanda=True, expandb=True
) -> Mesh:
...
def bearing_slot_exterior(
    ...
    return revolution(wire(profile).segmented(), Axis(O, -Z)).transform(
        translate(axis[0]) * mat4(quat(Z, axis[1]))
    )


def bearing_slot_interior(
    axis: Axis,
    dint: float,
    height: float,
    shouldering=True,
...
def circular_screwing(
    axis, radius, height, dscrew, diameters: int = 1, div: int = 8, hold: float = 0
) -> "(Mesh, list)":
    ...
    for i in range(diameters):
        holes += cylinder(
            a - gap, b + gap, enlarge * stfloor(0.8**i * dscrew) / 2
        ).transform(rotatearound(i * 1.4 * dscrew / radius, axis))
    ...
    if hold:
        ...
        holes += repeataround(
            screw_slot(
                Axis(a + gap, -z), dscrew, screw=height - hold, hole=hold, flat=True
            ),
            2,
...
def grooves_profile(
    radius, repetitions: int = 16, alignment=0.5, angle=radians(40)
) -> Wire:
    ...
    def profile(t, min=-1, max=1):
        return (radius + (0.5 - alignment) * h + h * sin(repetitions * t)) * vec3(
            cos(t), sin(t), 0
        )
    ...
    return wire(
        [
            profile(t, -1, 0.7)
            for t in linrange(0, 2 * pi, div=12 * repetitions, end=False)
        ]
    ).close()


def grooves(
    radius, height, repetitions: int = 16, alignment=0.5, angle=radians(40)
) -> Mesh:
    ...
    if alignment > 0.5:
        ...
        s = 1 - h / radius
    return extrans(
        grooves_profile(radius, repetitions, alignment, angle),
        [
            scale(vec3(s)),
            translate(h * Z),
...
def delta3(
    ...
    """
    if forearm_width is None:
        forearm_width = 0.1 * forearm
    ...
    for i, p in enumerate(arms):
        motors.append(
            Revolute(("base", f"backarm.{i}"), Axis(p, cross(Z, p * base)), Axis(O, X))
        )
        joints.append(
            Ball((f"backarm.{i}", f"forearm.{i}.0"), backarm * Z + forearm_width * X, O)
        )
        joints.append(
            Ball((f"backarm.{i}", f"forearm.{i}.1"), backarm * Z - forearm_width * X, O)
        )
        joints.append(
            Ball(
                ("tool", f"forearm.{i}.0"),
                p * tool + forearm_width * cross(Z, p),
                forearm * Z,
            )
        )
        joints.append(
