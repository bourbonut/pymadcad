...
def cache(
    filename: str, create: callable = None, name=None, storage=None, **opts
) -> Mesh:
...
def cachefunc(f):
    ...
    def repl(*args, **kwargs):
        ...
        key = "{}/{}{}-{}.pickle".format(
            ...
            hex(
                int.from_bytes(
                    md5(repr((*args, sorted(list(kwargs.items())))).encode()).digest(),
                    "little",
                )
            ),
        )
...
try:
    ...
    def ply_read(file, **opts):
        ...
        if "vertex" not in data:
            raise FileFormatError("file must have a vertex buffer")
        if "face" not in data:
            raise FileFormatError("file must have a face buffer")
    ...
    def ply_write(mesh, file, **opts):
        vertices = np.asarray(mesh.points).astype(
            np.dtype([("x", "f4"), ("y", "f4"), ("z", "f4")])
        )
        faces = np.empty(
            len(mesh.faces), dtype=[("vertex_indices", "u4", (3,)), ("group", "u2")]
        )
...
try:
    ...
    def stl_write(mesh, file, **opts):
        stlmesh = stl.mesh.Mesh(
            np.zeros(len(mesh.faces), dtype=stl.mesh.Mesh.dtype),
            name=mesh.options.get("name"),
        )
        stlmesh.vectors[:] = typedlist_to_numpy(mesh.points, "f4")[
            typedlist_to_numpy(mesh.faces, "i4")
        ]
...
def jsondecode(obj):
    if "type" in obj:
        ...
        if t in {"vec2", "vec3", "vec4", "mat2", "mat3", "mat4", "quat"}:
            ...
            return Mesh(
                [vec3(p) for p in obj["points"]],
                [tuple(f) for f in obj["faces"]],
                obj["tracks"],
                obj["groups"],
            )
        elif t == "Web":
