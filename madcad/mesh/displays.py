...
from ..mathutils import length, dot, normalize, fvec3, fvec4, fmat4
...
class MeshDisplay(Display):
    ...
    def __init__(self, scene, positions, normals, faces, lines, idents, color=None):
        ...
        self._disp_faces = FacesDisplay(
            scene, self._vertices, normals, faces, color=color, reflect=reflect, layer=0
        )
        self._disp_ghost = GhostDisplay(
            scene, self._vertices, normals, faces, color=line, layer=0
        )
        self._disp_groups = LinesDisplay(
            scene, self._vertices, lines, color=line, alpha=1, layer=-2e-6
        )
        self._disp_points = PointsDisplay(
            scene, self._vertices, range(len(positions)), color=line, layer=-3e-6
        )
        wire = []
        for f in faces:
            wire.append((f[0], f[1]))
            wire.append((f[1], f[2]))
            wire.append((f[2], f[0]))
        self._disp_wire = LinesDisplay(
            scene, self._vertices, wire, color=line, alpha=0.3, layer=-1e-6
        )
    ...
    def stack(self, scene):
        ...
        if scene.options["display_groups"]:
            yield (self, "screen", 1, self._disp_groups.render)
        if scene.options["display_points"]:
            yield (self, "screen", 2, self._disp_points.render)
        if scene.options["display_wire"]:
            yield (self, "screen", 2, self._disp_wire.render)
...
class WebDisplay(Display):
    ...
    def __init__(self, scene, positions, lines, points, idents, color=None):
        ...
        self._vertices = Vertices(scene.context, positions, idents)
        self._disp_edges = LinesDisplay(
            scene, self._vertices, lines, color=color, alpha=1, layer=-2e-6
        )
        self._disp_groups = PointsDisplay(scene, self._vertices, points, layer=-3e-6)
        self._disp_points = PointsDisplay(
            scene, self._vertices, range(len(positions)), layer=-1e-6
        )

    def stack(self, scene):
        yield (self, "screen", -1, self._vertices.prerender)
        if scene.options["display_groups"]:
            yield (self, "screen", 2, self._disp_groups.render)
        if scene.options["display_points"]:
            yield (self, "screen", 2, self._disp_points.render)
...
class Vertices:
    ...
    def prerender(self, view):
        if self.flags_updated:
            self.flags_updated = False
            self.u_flags = (None in self.hovered) * self.HOVERED | (
                None in self.selected
            ) * self.SELECTED
            if (None not in self.selected) or (None not in self.hovered):
                for i, ident in enumerate(self.idents):
                    self.v_flags[i] = (ident in self.hovered) * self.HOVERED | (
                        ident in self.selected
                    ) * self.SELECTED
...
class FacesDisplay:
    def __init__(self, scene, vertices, normals, faces, color, reflect, layer=0):
        ...
        def load(scene):
            img = Image.open(
                resourcedir + "/textures/" + settings.display["solid_reflect"]
            )
    ...
    def render(self, view):
        if self.va:
            ...
            self.va.program["min_color"].write(
                self.color * settings.display["solid_color_side"]
            )
            self.va.program["max_color"].write(
    ...
    def identify(self, view):
        if self.va:
            ...
            self.va_ident.program["view"].write(
                view.uniforms["view"] * self._vertices.world
            )
...
class LinesDisplay:
    def __init__(self, scene, vertices, lines, color, alpha=1, layer=0):
        ...
        if lines is not None and len(lines) and vertices.vb_positions:
            # allocate buffers
            self.vb_lines = scene.context.buffer(
                np.asarray(lines, dtype="u4", order="C")
            )
            self.va = scene.context.vertex_array(
                self.shader,
                [
...
class PointsDisplay:
    def __init__(self, scene, vertices, indices=None, color=None, ptsize=3, layer=0):
        ...
        if indices is not None and len(indices) and vertices.vb_positions:
            self.vb_indices = scene.context.buffer(
                np.asarray(indices, dtype="u4", order="C")
            )
            self.va = scene.context.vertex_array(
                self.shader,
                [
