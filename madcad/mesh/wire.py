...
class Wire(NMesh):
    ...
    def __init__(
        self, points=None, indices=None, tracks=None, groups=None, options=None
    ):
        self.points = ensure_typedlist(points, vec3)
        self.indices = ensure_typedlist(
            indices if indices is not None else range(len(self.points)), "I"
        )
    ...
    def __getitem__(self, i):
        ...
        if isinstance(i, Integral):
            return self.points[self.indices[i]]
        elif isinstance(i, slice):
            return typedlist((self.points[j] for j in self.indices[i]), dtype=vec3)
        else:
            raise TypeError("item index must be int or slice")
        ...
        if isinstance(other, Wire):
            ...
            if self.groups is other.groups:
                if self.tracks or other.tracks:
                    ...
                    self.tracks.extend(
                        other.tracks or typedlist.full(0, len(other.indices), "I")
                    )
            else:
                lg = len(self.groups)
    ...
    def check(self):
        ...
        if not (isinstance(self.indices, typedlist) and self.indices.dtype == "I"):
            raise MeshError("indices must be a typedlist(dtype='I')")
        if self.tracks and not (
            isinstance(self.tracks, typedlist) and self.tracks.dtype == "I"
        ):
        ...
        for i in self.indices:
            if i >= l:
                raise MeshError(
                    "some indices are greater than the number of points", i, l
                )
        if self.tracks:
            if len(self.indices) != len(self.tracks):
                raise MeshError("tracks list doesn't match indices list length")
            if max(self.tracks) >= len(self.groups):
                raise MeshError(
                    "some tracks are greater than the number of groups",
                    max(self.tracks),
                    len(self.groups),
                )
    ...
    def edges(self) -> typedlist:
        ...
        return typedlist(
            (self.edge(i) for i in range(len(self.indices) - 1)), dtype=uvec2
        )
    ...
    def barycenter(self) -> vec3:
        ...
        if len(self.indices) == 1:
            return self.points[self.indices[0]]
        acc = vec3(0)
        tot = 0
        for i in range(1, len(self)):
            a, b = self[i - 1], self[i]
    ...
    def vertexnormals(self, loop=False):
        ...
        for i in range(len(self.indices)):
            ...
            normals[i - 1] = normalize(
                normalize(self.points[b] - self.points[a])
                + normalize(self.points[b] - self.points[c])
            )
    ...
    def tangents(self, loop=False):
        ...
        for i in range(len(self.indices)):
            a, b, c = self.indices[i - 2], self.indices[i - 1], self.indices[i]
            tangents[i - 1] = normalize(
                cross(self.points[b] - self.points[a], self.points[b] - self.points[c])
            )
    ...
    def _make_loop_consistency(self, normals, loop):
        ...
        if loop:
            ...
            dir = (
                self[l // 2] - self[0]
            )  # WARNING: if those two points are too close the computation is misleaded
    ...
    def unclose(self) -> "Self":
        ...
        if self.isclosed():
            return Wire(
                self.points, self.indices[:-1], self.tracks, self.groups, self.options
            )
        return self
    ...
    def __repr__(self):
        return "<Wire with {} points at 0x{:x}, {} indices>".format(
            len(self.points), id(self.points), len(self.indices)
        )
