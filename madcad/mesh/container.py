...
class NMesh(object):
    ...
    def pointat(self, point: vec3, neigh=NUMPREC) -> int:
        ...
        for i, p in enumerate(self.points):
            if distance(p, point) <= neigh:
                return i

    def pointnear(self, point: vec3) -> int:
        """Return the nearest point the the given location"""
        return min(range(len(self.points)), lambda i: distance(self.points[i], point))
    ...
    def qualify(self, *quals, select=None, replace=False) -> "self":
        ...
        """
        if select is None:
            it = range(len(self.groups))
        else:
            it = self.qualified_groups(select)
        for i in it:
            group = self.groups[i]
            if group is None:
                self.groups[i] = group = {}
...
def numpy_to_typedlist(array: "ndarray", dtype) -> "typedlist":
    ...
    if ndtype.fields:
        return typedlist(
            rfn.unstructured_to_structured(array).astype(ndtype, copy=False), dtype
        )
    else:
