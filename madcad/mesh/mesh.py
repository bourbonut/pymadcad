...
class Mesh(NMesh):
    ...
    def __init__(self, points=None, faces=None, tracks=None, groups=None, options=None):
        ...
        self.tracks = ensure_typedlist(
            tracks if tracks is not None else typedlist.full(0, len(self.faces), "I"),
            "I",
        )
        self.groups = (
            groups
            if groups is not None
            else [None] * (max(self.tracks, default=-1) + 1)
        )
    ...
    def check(self):
        ...
        if not (isinstance(self.points, typedlist) and self.points.dtype == vec3):
            raise MeshError("points must be a typedlist(dtype=vec3)")
        if not (isinstance(self.faces, typedlist) and self.faces.dtype == uvec3):
            raise MeshError("faces must be a typedlist(dtype=uvec3)")
        if not (isinstance(self.tracks, typedlist) and self.tracks.dtype == "I"):
            raise MeshError("tracks must be a typedlist(dtype='I')")
        l = len(self.points)
        for face in self.faces:
            for p in face:
                if p >= l:
                    raise MeshError(
                        "some point indices are greater than the number of points",
                        face,
                        l,
                    )
            if face[0] == face[1] or face[1] == face[2] or face[2] == face[0]:
                raise MeshError("some faces use the same point multiple times", face)
        if len(self.faces) != len(self.tracks):
            raise MeshError("tracks list doesn't match faces list length")
        if max(self.tracks, default=-1) >= len(self.groups):
            raise MeshError(
                "some face group indices are greater than the number of groups",
                max(self.tracks, default=-1),
                len(self.groups),
            )
    ...
    def facenear(self, point) -> int:
        ...
        return min(
            range(len(self.faces)), key=lambda i: distance_pt(point, self.facepoints(i))
        )
        ...
        for face in self.faces:
            ...
            for i in range(3):
                ...
                if face[i] not in border:
                    ...
                    contrib = anglebt(
                        self.points[face[i - 2]] - o, self.points[face[i - 1]] - o
                    )
    ...
    def tangents(self) -> "{int: vec3}":
        ...
        for face in self.faces:
            for e in ((face[0], face[1]), (face[1], face[2]), (face[2], face[0])):
                if e in edges:
                    ...
                    edges[(e[1], e[0])] = self.facenormal(face)
        ...
        for loop in suites(edges, cut=False):
            ...
            for i in range(len(loop)):
                ...
                o = cross(
                    self.points[loop[i - 2]] - self.points[loop[i]],
                    edges[(loop[i - 2], loop[i - 1])] + edges[(loop[i - 1], loop[i])],
                )
                tangents[loop[i - 1]] = normalize(
                    mix(o, c, clamp(length2(c) / length2(o) / NUMPREC, 0, 1))
                )
        return tangents
        ...
        if args:
            ...
            for arg in args:
                if arg is None:
                    ...
                    groups.update(self.qualified_groups(arg))
        else:
            groups = None
        ...
        for i, face in enumerate(self.faces):
            ...
            if groups is None or None in groups or track in groups:
                for i in range(len(face)):
                    ...
                    if e in belong:
                        if belong[e] != track and (
                            groups is None or track in groups and belong[e] in groups
                        ):
    ...
    def propagate(self, atface, atisland=None, find=None, conn=None):
        ...
        while True:
            ...
            while stack:
                i = stack.pop()
                if reached[i]:
                    continue  # make sure this face has not been stacked twice
                reached[i] = True
                atface(i, reached)
                f = self.faces[i]
                for i in range(3):
    ...
    def splitgroups(self, edges=None):
        ...
        idents = typedlist.full(
            0, len(self.points), "I"
        )  # track id corresponding to each point
        ...
        faces = typedlist(
            (
                uvec3(repl(a, t), repl(b, t), repl(c, t))
                for (a, b, c), t in zip(self.faces, self.tracks)
            ),
            dtype=uvec3,
        )
    ...
    def islands(self, conn=None) -> "[Mesh]":
        ...
        def atisland(reached):
            islands.append(
                Mesh(self.points, deepcopy(faces), deepcopy(tracks), self.groups)
            )
            faces.clear()
            tracks.clear()
    ...
    def display(self, scene):
        ...
        for edge, f1 in conn.items():
            ...
            if f2 is None:
                continue
            if (
                m.tracks[f1] != m.tracks[f2]
                or dot(m.facenormal(f1), m.facenormal(f2)) <= thresh
            ):
                tosplit.append(edge)
    ...
    def __repr__(self):
        return "<Mesh with {} points at 0x{:x}, {} faces>".format(
            len(self.points), id(self.points), len(self.faces)
        )
...
def mkquad(mesh, pts, track=0):
    ...
    if distance2(mesh.points[pts[0]], mesh.points[pts[2]]) <= distance2(
        mesh.points[pts[1]], mesh.points[pts[3]]
    ):
