...
def distance2_pm(point, mesh) -> "(d, prim)":
    ...
    if isinstance(mesh, Mesh):
        for face in mesh.faces:
            ...
            for i in range(3):
                ...
                if dot(cross(n, d), point - f[i - 2]) < 0:
                    ...
                    if x < 0:
                        dist, candidate = distance2(point, f[i - 2]), face[i - 2]
                    elif x > 1:
                        dist, candidate = distance2(point, f[i - 1]), face[i - 1]
                    else:
                        dist, candidate = (
                            length2(noproject(point - f[i - 2], d)),
                            (face[i - 2], face[i - 1]),
                        )
                    plane = False
                    break
        ...
        for edge in edges:
            ...
            # check if closer to the edge points than to the edge axis
            if x < 0:
                dist, candidate = distance2(point, e[0]), edge[0]
            elif x > 1:
                dist, candidate = distance2(point, e[1]), edge[1]
            else:
                dist, candidate = length2(noproject(point - e[0], d)), edge
            if dist < score:
                best, score = candidate, dist
...
def mesh_distance(m0, m1) -> "(d, prim0, prim1)":
    ...
    def analyse(m, o):
        ...
        return min(((*distance2_pm(m.points[i], o), i) for i in it), key=lambda t: t[0])

    # symetrical evaluation
    d0 = analyse(m0, m1)
    d1 = analyse(m1, m0)
    if d0[0] < d1[0]:
        return (sqrt(d0[0]), d0[2], d0[1])
    else:
        return (sqrt(d1[0]), d1[1], d1[2])
