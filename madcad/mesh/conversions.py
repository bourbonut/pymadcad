...
def wire(*arg, resolution=None) -> Wire:
    ...
    if isinstance(arg, Wire):
        ...
        if len(indices) > 1:
            raise ValueError("the given web has junctions or is discontinuous")
        return Wire(
            arg.points, indices[0], groups=[None]
        )  # TODO: find a way to get the groups from the Web edges through suites or not
    elif hasattr(arg, "mesh"):
