...
class Web(NMesh):
    ...
    def __init__(self, points=None, edges=None, tracks=None, groups=None, options=None):
        ...
        self.tracks = ensure_typedlist(
            tracks or typedlist.full(0, len(self.edges), "I"), "I"
        )
        self.groups = (
            groups
    ...
    def check(self):
        ...
        if not (isinstance(self.edges, typedlist) and self.edges.dtype == uvec2):
            raise MeshError("edges must be in a typedlist(dtype=uvec2)")
        if not (isinstance(self.tracks, typedlist) and self.tracks.dtype == "I"):
            raise MeshError("tracks must be in a typedlist(dtype='I')")
        l = len(self.points)
        for line in self.edges:
            for p in line:
                if p >= l:
                    raise MeshError(
                        "some indices are greater than the number of points", line, l
                    )
                if p < 0:
                    raise MeshError("point indices must be positive", line)
            if line[0] == line[1]:
                raise MeshError("some edges use the same point multiple times", line)
        if len(self.edges) != len(self.tracks):
            raise MeshError("tracks list doesn't match edge list length")
        if max(self.tracks, default=-1) >= len(self.groups):
            raise MeshError(
                "some line group indices are greater than the number of groups",
    ...
    def edgenear(self, point: vec3) -> int:
        ...
        return min(
            range(len(self.edges)), key=lambda i: distance_pe(point, self.edgepoints(i))
        )
    ...
    def frontiers(self, *args) -> "Wire":
        ...
        for i, edge in enumerate(self.edges):
            ...
            if groups is None or None in groups or track in groups:
                for p in edge:
                    if p in belong:
                        if belong[p] != track and (
                            groups is None or track in groups and belong[p] in groups
                        ):
    ...
    def arcs(self) -> "[Wire]":
        ...
        return [
            Wire(self.points, typedlist(loop, dtype=uvec2))
    ...
    def __repr__(self):
        return "<Web with {} points at 0x{:x}, {} edges>".format(
            len(self.points), id(self.points), len(self.edges)
        )
        ...
        for group in range(len(self.groups)):
            ...
            for edge, track in zip(self.edges, self.tracks):
                if track != group:
                    continue
                edges.append(
                    uvec2(usept(edge[0], track, used), usept(edge[1], track, used))
                )
                for p in edge:
                    if p in frontier:
                        frontier.remove(p)
