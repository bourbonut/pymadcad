...
def processalign(align, size):
    ...
    if isinstance(align, str):
        if align in ("left", "top"):
            ...
            raise ValueError(
                "align must be int, float or any of 'left', 'right', 'center'"
            )
    else:
        return align
...
class BezierCubic:
    ...
    def mesh(self, resolution=None):
        div = settings.curve_resolution(
            ...
            + distance(self.c, self.d),
            anglebt(self.b - self.a, self.c - self.b)
            + angletbt(self.c - self.b, self.d - self.c),
            resolution,
        )
        return Wire(self(t) for t in linrange(0, 1, div=div))
...
try:
    ...
    def character_primitives(face: freetype.Face) -> list:
        ...
        face.glyph.outline.decompose(
            move_to=move_to, line_to=line_to, conic_to=conic_to, cubic_to=cubic_to
        )
        return primitives

    def ft2vec(ft):
        return vec3(ft.x, ft.y, 0)
    ...
    def character_cbox(face: freetype.Face) -> Box:
        ...
        return Box(min=vec3(box.xMin, box.yMin, 0), max=vec3(box.xMax, box.yMax, 0))
    ...
    def character_outline(char: str, font: str, resolution=None) -> dict:
        ...
        try:
            ...
            if font not in cache_fonts:
                cache_fonts[font] = {}
            if char not in cache_fonts[font]:
                cache_fonts[font][char] = {}
            ...
            cache["web"] = (
                web(character_primitives(face), resolution=resolution)
                .transform(1 / scale)
                .mergegroups()
            )
            return cache
