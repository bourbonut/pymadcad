...
def skeleting(outline: Wire, skeleting: callable, prec=NUMPREC) -> [vec2]:
    ...
    def eval_intersect(i):
        ...
        if norminf(v1) < prec:
            v1 = perp(
                enormals[b1]
            )  # if edges are parallel, take the direction to the shape
        if norminf(v2) < prec:
            v2 = -perp(enormals[b2])
...
def triangulation_skeleton(outline: Wire, prec=NUMPREC) -> Mesh:
    ...
    for a, b in skeleton:
        if distance(pts[a], pts[b]) < minbone:
            if a not in merges:
                merges[a] = merges.get(b, b)
            elif b not in merges:
                merges[b] = merges.get(a, a)
    ...
    while len(hole) > 2:
        ...
        if scores[i] == -inf:
            raise TriangulationError(
                "no more feasible triangles (algorithm failure or bad input outline)",
                [outline.indices[i] for i in hole],
            )
...
def sweepline_loops(lines: Web, normal=None):
    ...
    for i, (a, b) in enumerate(edges):
        if pts[a][0] < pts[b][0]:
            edges[i] = b, a
    stack = sorted(
        edges, key=lambda e: (pts[e[0]][0], abs(orthoproj(pts[e[1]] - pts[e[0]])))
    )
    ...
    while stack:
        ...
        while i >= 0 and pts[stack[i][0]][0] == pts[edge[0]][0]:
            ...
            if e[0] == edge[0]:
                diff = abs(
                    orthoproj(pts[e[1]] - pts[e[0]])
                    - orthoproj(pts[edge[1]] - pts[edge[0]])
                )
                if diff > sc:
                    sc = diff
                    m = i
        ...
        if m is not None:
            ...
            coedge = stack.pop(m)
            if orthoproj(pts[edge[1]] - pts[edge[0]]) < orthoproj(
                pts[coedge[1]] - pts[coedge[0]]
            ):
        ...
        for i, (l0, l1) in enumerate(clusters):
            ...
            # interior hole that touch the outline
                ...
                break
            # interior hole
            elif coedge and affiney(l0, p0[0]) < p0[1] and affiney(l1, p0[0]) > p0[1]:
                clusters[i] = (l0, coedge)
                clusters.insert(i, (edge, l1))
        ...
        if not found:
            ...
            # if it's an ambiguous edge without use for now, restack it
                ...
                stack.insert(-1, edge)
                if coedge:
                    stack.insert(-1, coedge)
            else:
                raise TriangulationError(
                    "algorithm failure, can be due to the given outline", edge, coedge
                )
...
def discretise_refine(
    curve: "[(x, f(x))]", func: "f(float) -> float", resolution=None, simplify=True
):
    ...
    pts = [
        curve[0],
        curve[1],
    ]  # use a new list to improve insertion efficiencies (we will always insert close to the end)
    i = 2  # index in pts
    j = 2  # index in curve
    target = len(curve)
    ...
    i = 4
    while i < len(pts):
        if goodcorner(pts[i - 4], pts[i - 3], pts[i - 1]) and goodcorner(
            pts[i - 3], pts[i - 1], pts[i]
        ):
            pts.pop(i - 2)
        else:
            i += 1
