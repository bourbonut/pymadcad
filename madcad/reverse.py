...
def segmentation(mesh, tolerance=5, sharp=0.2) -> Mesh:
    ...
    prec = (
        1 / mesh.maxnum()
    )  # curvature precision, curvature is rad/m, so 1rad/max dist seems to be a fairly low curvature
    ...
    edgecurve = {}
    facecurve = [0.0] * len(mesh.faces)
    weight = [0.0] * len(mesh.faces)
    for e, fa in conn.items():
        if edgekey(*e) in edgecurve:
            continue
        fb = conn.get((e[1], e[0]))
        if not fb:
            continue
        ...
        if sharp > angle:
            ...
            facecurve[fa] += (
                curve * w
            )  # multiply angle at edge by   (triangle height) ~= surface/(edge length)
    ...
    for i, f in enumerate(mesh.faces):
        area = (
            0.5
            * length(cross(pts[f[1]] - pts[f[0]], pts[f[2]] - pts[f[0]]))
            * weight[i]
        )
        if area:
            facecurve[i] /= area
    ...
    while True:
        ...
        for index in range(index, len(sortedfaces)):
            if tracks[sortedfaces[index]] == -1:
                break
        if index >= len(sortedfaces):
            break
        i = sortedfaces[index]
        f = mesh.faces[i]
        ...
        while front:
            ...
            if abs(facecurve[j] - estimate) / (abs(estimate) + prec) > tolerance:
                continue
...
def guessjoint(
    solid1, solid2, surface1, surface2, guess=quat(), precision=1e-5
) -> "joint":
...
def guesssurface(surface, precision=1e-5, attempt=False):
    ...
    solverargs = dict(ftol=precision, max_nfev=300, method="lm")
    ...
    def evaluate(x):
        ...
        for i, f in enumerate(surface.faces):
            ...
            for j, p in enumerate(fp):
                residual[i, j + 1] = (
                    length(noproject(p - origin, direction)) - radius
                ) ** 2
        return residual.ravel()
    ...
    for e in conn:
        if (e[1], e[0]) in conn and e[0] < e[1]:
            edge = cross(
                surface.facenormal(conn[e]), surface.facenormal(conn[(e[1], e[0])])
            )
