...
class PointDisplay(Display):
    ...
    def _shared(self, scene):
        ...
        vb = scene.context.buffer(
            np.array([(0, 0), (0, 1), (1, 1), (0, 0), (1, 1), (1, 0)], "f4")
        )

        return dict(
            _texture=texture,
            _va_screen=scene.context.vertex_array(
                shader, [(vb, "2f", "v_uv")], mode=mgl.TRIANGLES
            ),
            _va_ident=scene.context.vertex_array(
                ident_shader, [(vb, "2f", "v_uv")], mode=mgl.TRIANGLES
            ),
        )

    def stack(self, scene):
        return ((self, "ident", 2, self._identify), (self, "screen", 2, self._render))

    def _render(self, view):
        self._va_screen.program["color"].write(highlight_color(self, self.color))
        self._va_screen.program["position"].write(
            fvec3(self.world * fvec4(self.position, 1))
        )
    ...
    def _identify(self, view):
        ...
        self._va_ident.program["position"].write(
            fvec3(self.world * fvec4(self.position, 1))
        )
...
class AxisDisplay(Display):
    ...
    def _shared(self, scene):
        ...
        return dict(
            _va_screen=scene.context.vertex_array(
                shader, [(vb, "f f", "v_absciss", "v_alpha")], mode=mgl.LINES
            ),
            _va_ident=scene.context.vertex_array(
                ident_shader, [(vb, "f 4x", "v_absciss")], mode=mgl.LINES
            ),
        )
...
class AnnotationDisplay(Display):
    ...
    def __init__(self, scene, points, color):
        ...
        self._va_screen = scene.context.vertex_array(
            shader_screen, [(vb, "3f f", "v_position", "v_alpha")], mode=mgl.LINES
        )
        self._va_ident = scene.context.vertex_array(
            shader_ident, [(vb, "3f 4x", "v_position")], mode=mgl.LINES
        )
...
class SplineDisplay(Display):
    ...
    def __init__(self, scene, handles, curve, color=None):
        ...
        self._va_curve = ctx.vertex_array(
            shader, [(vb_curve, "3f4", "v_position")], mode=mgl.LINE_STRIP
        )

        shader = scene.share(load_shader_ident, load_shader_ident)
        self._va_ident = ctx.vertex_array(
            shader, [(vb_curve, "3f4", "v_position")], mode=mgl.LINE_STRIP
        )

    def stack(self, scene):
        return ((self, "screen", 1, self._render), (self, "ident", 1, self._identify))

    def _render(self, view):
        self._va_handles.program["color"].write(
            fvec4(highlight_color(self, settings.colors["annotation"]), 0.6)
        )
        self._va_curve.program["color"].write(
            fvec4(highlight_color(self, self.color), 1)
        )
        ...
        self._va_handles.program["color"].write(
            fvec4(highlight_color(self, self.color_handles), 0.5)
        )
...
class GridDisplay(Display):
    ...
    def __init__(self, scene, center=None, unit=1.0, color=None, contrast=1.8):
        self.unit = unit
    ...
    def _share(self, scene):
        ...
        return scene.context.vertex_array(
            shader, [(vb, "2f4 f2", "v_position", "v_opacity")], mode=mgl.POINTS
        )
...
def tupledisplay(scene: Scene, t: tuple) -> Display:
    if primitives.isaxis(t):
        return AxisDisplay(scene, t)
