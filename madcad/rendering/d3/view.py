...
class GLView3D:
    ...
    def render(
        self, size: uvec2 = None, view: fmat4 = None, proj: fmat4 = None
    ) -> Self:
    ...
    def _reallocate(self, size):
        ...
        self._screen_multisample = ctx.simple_framebuffer(
            size, components=3, samples=4, dtype="f1"
        )
    ...
    def _setup_screen(self):
        ...
        if len(background) == 3:
            ...
            raise ValueError(
                f"background_color must be a RGB or RGBA tuple, currently {background}"
            )
    ...
    class QView3D(QWidget):
        ...
        def _reallocate(self, size):
            ...
            if (
                ...
                and w == self.color.shape[1]
                and h == self.color.shape[0]
            ):
                return
        ...
        def paintEvent(self, painter):
            ...
            self.gl.render(
                ...
                proj=self.projection.matrix(
                    w / h if h > 0 else 0, self.navigation.distance
                ),
            )
            self._reallocate(size)
            # retreive buffer
        ...
        def changeEvent(self, evt):
            # detect theme change
            if (
                evt.type() == QEvent.Type.PaletteChange
                and settings.display["system_theme"]
            ):
        ...
        def _selection(self, disp, sub, evt):
            ...
            if (
                evt.type() == QEvent.Type.MouseButtonRelease
                and evt.button() == Qt.MouseButton.LeftButton
                and hasattr(disp, "selected")
            ):
                shift = bool(
            ...
            if evt.type() == QEvent.Type.MouseMove and hasattr(disp, "hovered"):
                ...
                if disp is not self.scene.hover or (
                    isinstance(disp.hovered, set) and sub not in disp.hovered
                ):
        ...
        def _move_mode(self):
            ...
            while True:
                ...
                if (
                    evt.type() == QEvent.Type.MouseButtonPress
                    and evt.button() == Qt.MouseButton.MiddleButton
                ):
                    self._mode = self._ROTATE
                else:
                    if ctrl and alt:
                        self._mode = self._ZOOM
                    elif ctrl:
                        self._mode = self._PAN
                    elif alt:
                        self._mode = self._ROTATE
                    else:
                        self._mode = None
            ...
            while True:
                ...
                if evt.type() == QEvent.Type.Wheel:
                    self.navigation.zoom(
                        exp(-evt.angleDelta().y() / (8 * 90))
                    )  # the 8 factor is there because of the Qt documentation
                    evt.accept()
                    self.update()
                ...
                if self._mode and evt.type() == QEvent.Type.MouseButtonPress:
                    ...
                    while True:
                        ...
                        if (
                            not self._mode
                            or evt.type() == QEvent.Type.MouseButtonRelease
                            or isinstance(evt, QMouseEvent)
        ...
        def _touch_move(self):
            ...
            while True:
                ...
                if isinstance(evt, QTouchEvent):
                    ...
                    if len(pts) == 2:
                        startlength = (
                            pts[0].lastPos() - pts[1].lastPos()
                        ).manhattanLength()
                        zoom = (
                            startlength
                            / (pts[0].pos() - pts[1].pos()).manhattanLength()
                        )
                        displt = (
                            (pts[0].pos() + pts[1].pos()) / 2
                            - (pts[0].lastPos() + pts[1].lastPos()) / 2
                        ) / self.height()
                    ...
                    # view translation
                        ...
                        lc = (
                            pts[0].lastPos() + pts[1].lastPos() + pts[2].lastPos()
                        ) / 3
                        lr = (
                        ...
                        cc = (pts[0].pos() + pts[1].pos() + pts[2].pos()) / 3
                        cr = (
        ...
        def ptat(self, point: QPoint) -> fvec3 | None:
            ...
            if depthred == 1.0:
                ...
                depth = (
                    b / (depthred + a) * 0.5
                )  # TODO get the true depth  (can't get why there is a strange factor ... opengl trick)
                ...
                return fvec3(
                    affineInverse(view)
                    * fvec4(depth * x / proj[0][0], depth * y / proj[1][1], -depth, 1)
                )
        ...
        def displayat(self, point: QPoint) -> (Display, int) | None:
            ...
            if ident and "ident" in self.scene.stacks:
                ...
                while rdri > 0 and self.gl._steps[rdri - 1] == ident:
                    rdri -= 1
                if rdri > 0:
                    subi = ident - self.gl._steps[rdri - 1] - 1
                else:
                    subi = ident - 1

                if rdri >= len(self.scene.stacks["ident"]):
                    print(
                        "wrong identification index",
                        ident,
                        self.scene.stacks["ident"][-1],
                    )
                    nprint(self.gl._steps)
                    return
        ...
        def adjust(self, box: Box = None):
            ...
            dist = length(noproject(box.center - camera, look)) + max(
                glm.abs(box.width)
            ) / 2 * (1 + margin)
            if not dist > 1e-6:
                return
...
class Orbit:
    ...
    def __init__(
        self, position: fvec3 = 0, distance: float = 1, orient: fvec3 = fvec3(1, 0, 0)
    ):
    ...
    def rotate(self, offset: vec3):
        ...
        self.orient = (
            inverse(fquat(fvec3(-offset.y, -offset.dx, offset.dz) * pi)) * self.orient
        )
...
class Turntable:
    ...
    def __init__(
        self, position: fvec3 = 0, distance: float = 1, yaw: float = 0, pitch: float = 0
    ):
...
class Perspective:
    ...
    def matrix(self, ratio, distance) -> fmat4:
        return perspective(
            self.fov / min(1, ratio), ratio, distance * 1e-2, distance * 1e4
        )
...
class Orthographic:
    ...
    def matrix(self, ratio, distance) -> fmat4:
        ...
        return fmat4(
            min(1, 1 / ratio) / (distance * self.size),
            0,
            0,
            0,
            0,
            min(1, ratio) / (distance * self.size),
            0,
            0,
            0,
            0,
