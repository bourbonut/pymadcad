...
class Scene:
    ...
    def __init__(
        ...
        if context:
            ...
            self.context = global_context = mgl.create_standalone_context(
                requires=opengl_version
            )
    ...
    def display(self, obj, former: Display = None) -> Display:
        ...
        ido = id(obj)
        assert ido not in self._memo, (
            "there should not be recursion loops in cascading displays"
        )
        self._memo.add(ido)
        ...
        try:
            ...
            # create it with the displayable protocol
                ...
                if isinstance(obj.display, type):
                    ...
                    raise TypeError(
                        "member 'display' must be a method or a type, on {}".format(
                            type(obj).__name__
                        )
                    )
            else:
            ...
            if not isinstance(disp, Display):
                raise TypeError(
                    "the display for {} is not a subclass of Display: {}".format(
                        type(obj).__name__, type(disp)
                    )
                )
    ...
    def share(self, key, generator=None):
        ...
        if key in self.shared:
            ...
            raise KeyError(
                "resource {} doesn't exist or is not loaded".format(repr(key))
            )
            ...
            with self.context:
                ...
                for step in self.root.stack(self):
                    if isinstance(step, tuple):
                        ...
                        raise TypeError(
                            "stack step must be of type Step, not {}".format(type(step))
                        )
                    if not isinstance(step.display, Display):
                        raise TypeError(
                            "step.display must be a Display, not {}".format(
                                type(step.display)
                            )
                        )
    ...
    def prepare(self, scene):
        ...
        if self._pending is not None:
            ...
            for key in self.displays:
                if key not in self._pending or not scene.displayable(
                    self._pending[key]
                ):
                    removed.append(key)
            for key in removed:
            ...
            for key, displayable in self._pending.items():
                if not scene.displayable(displayable):
                    continue
                try:
                    self.displays[key] = scene.display(
                        displayable, self.displays.get(key)
                    )
                except:
