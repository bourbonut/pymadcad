...
class CheapMap:
    ...
    def region(self, viewport: tuple) -> ndarray:
        ...
        """
        if (
            viewport[0] < self.viewport[0]
            or viewport[2] > self.viewport[2]
            or viewport[1] < self.viewport[1]
            or viewport[3] > self.viewport[3]
        ):
            ...
            self.framebuffer.read_into(
                ...
                (
                    viewport[0],
                    self.framebuffer.height - viewport[3],
        ...
        view = view.reshape(
            (self.viewport[3] - self.viewport[1], self.viewport[2] - self.viewport[0])
        )
        view = view[::-1]
        return view[
...
def highlight_color(display: Display, color: fvec3) -> fvec3:
    if display.selected:
        highlight = fvec4(settings.display["selection_color"])
    elif display.hovered:
        highlight = fvec4(settings.display["hover_color"])
    else:
        highlight = fvec4(0)
...
try:
    ...
    def vec_to_qpoint(p: ivec2) -> QPoint:
        return QPoint(p.x, p.y)

    def qpoint_to_vec(p: QPoint) -> ivec2:
        return ivec2(p.x(), p.y())

    def vec_to_qsize(p: uvec2) -> QSize:
        return QSize(p.x, p.y)

    def qsize_to_vec(p: QSize) -> uvec2:
        return uvec2(p.width(), p.height())
