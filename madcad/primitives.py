...
class ArcCentered(object):
    ...
    def fit(self):
        return (
            dot(self.a - self.axis[0], self.axis[1]) ** 2,
...
class ArcTangent(object):
    ...
    def center(self):
        ...
        return (
            mix(unproject(self.a - self.b, n), unproject(self.c - self.b, n), 0.5)
            + self.b
        )
...
class TangentEllipsis(object):
    ...
    def mesh(self, resolution=None):
        ...
        div = settings.curve_resolution(
            distance(self.a, self.c), anglebt(x, -y), self.resolution or resolution
        )
        pts = [self.a]
...
class Circle(object):
    ...
    def __init__(
        self, axis: Axis, radius: float, alignment=vec3(1, 0, 0), resolution=None
    ):
    ...
    def mesh(self, resolution=None):
        ...
        angle = 2 * pi
        div = settings.curve_resolution(
            angle * self.radius, angle, self.resolution or resolution
        )
        return mesh.Wire(
            typedlist(
...
class Ellipsis(object):
    ...
    def mesh(self, resolution=None):
        ...
        div = settings.curve_resolution(
            angle * radius, angle, self.resolution or resolution
        )
        return mesh.Wire(
            typedlist(
...
class Interpolated(object):
    ...
    def mesh(self, resolution=None):
        ...
        tas = [
            self.weights[i - 1]
            * length(pts[i] - pts[i - 1])
            * normalize(pts[i] - pts[i - 2])
            for i in range(2, len(pts))
        ]
        tbs = [
            self.weights[i - 1]
            * length(pts[i - 2] - pts[i - 1])
            * normalize(pts[i - 2] - pts[i])
            for i in range(2, len(pts))
        ]
    ...
    def display(self, scene):
        ...
        return SplineDisplay(
            scene,
            typedlist_to_numpy(self.points, np.float32),
            typedlist_to_numpy(self.mesh().points, np.float32),
        )
...
class Softened(object):
    ...
    def mesh(self, resolution=None):
        ...
        # find midpoints
        mid = [
            (self.weights[i - 1] * pts[i - 1] + self.weights[i] * pts[i])
        ...
        for i in range(len(pts) - 2):
            ...
            # get resolution
            div = 1 + settings.curve_resolution(
                length(ta) + length(tb), anglebt(ta, -tb), self.resolution or resolution
            )
